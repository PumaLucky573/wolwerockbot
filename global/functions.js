...
module.exports = (bot, utils, ytdl, config) => {
    ...
    fs.readdir("./commands/", (err, files) => {
        ...
        if (jsfiles.length <= 0) return console.log("Da sind keine Befehle zu laden...");

        console.log(`Lade ${jsfiles.length} Befehle...`);
        jsfiles.forEach((f, i) => {
    ...
    bot.loadCommand = (commandName) => {
        try {
            ...
            return utils.cmd_fail(`Error: ${err}\nBefehl \`${commandName}\` kann nicht gefunden werden.`, `${config.prefix}reload <command>`);
        }
    };

    bot.unloadCommand = async (commandName) => {
        try {
            if (!commandName) return `Der Befehl \`${commandName}\` scheint nicht zu existieren. Versuche es nochmal!`;
    ...
    bot.handleVideo = async (video, message, vc, playlist = false) => {
        ...
        if (!queue) {
            ...
            try {
                ...
                console.error(`Ich konnte deinem Voice-Channel nicht beitreten: ${err}`);
            }
        } else {
            queue.musics.push(music);
            if (playlist) return;
            else return message.channel.send(`🎵 **${music.title}** wurde der Warteschlange hinzugefügt.`).then(msg => msg.delete(5000));
        }
        return;
    }
    ...
    bot.play = (guild, music) => {
        ...
        if (!music) {
            ...
            return queue.textChannel.send(`🎵 Warteschlange ist leer ich Disconnecte den Channel.`).then(msg => msg.delete(5000));
        }
        ...
        queue.textChannel.send(`🎵 **${music.title}** wird nun abgespielt.`).then(msg => msg.delete(5000));
    }
